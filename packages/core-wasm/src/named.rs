...
static NAMED_MAP: LazyLock<HashMap<&'static str, [u8; 3]>> = LazyLock::new(|| {
    let mut map = HashMap::with_capacity(NAMED_PAIRS.len());
    for &(name, rgb) in NAMED_PAIRS {
        map.insert(name, rgb);
    }
    map
});
